...
module.exports = class extends Base {
  ...
  async detailAction() {
    ...
    const attribute = await this.model('goods_attribute').field('nideshop_goods_attribute.value, nideshop_attribute.name').join('nideshop_attribute ON nideshop_goods_attribute.attribute_id=nideshop_attribute.id').order({
      'nideshop_goods_attribute.id': 'asc'
    }).where({
      'nideshop_goods_attribute.goods_id': goodsId
    }).select();
    const issue = await this.model('goods_issue').select();
    const brand = await this.model('brand').where({
      id: info.brand_id
    }).find();
    const commentCount = await this.model('comment').where({
      value_id: goodsId,
      type_id: 0
    }).count();
    const hotComment = await this.model('comment').where({
      value_id: goodsId,
      type_id: 0
    }).find();
    let commentInfo = {};
    if (!think.isEmpty(hotComment)) {
      const commentUser = await this.model('user').field(['nickname', 'username', 'avatar']).where({
      ...
      commentInfo = {
        ...
        pic_list: await this.model('comment_picture').where({
  ...
  async categoryAction() {
    ...
    const parentCategory = await model.where({
      id: currentCategory.parent_id
    }).find();
    const brotherCategory = await model.where({
  ...
  async listAction() {
    ...
    if (!think.isEmpty(categoryIds)) {
      // 查找二级分类的parent_id
      const parentIds = await this.model('category').where({
        id: {
          'in': categoryIds
        }
      }).getField('parent_id', 10000);
      // 一级分类
      const parentCategory = await this.model('category').field(['id', 'name']).order({
        'sort_order': 'asc'
      }).where({
        'id': {
          'in': parentIds
  ...
  async relatedAction() {
    ...
    const model = this.model('goods');
    const goodsId = this.get('id');
    const relatedGoodsIds = await this.model('related_goods').where({
      goods_id: goodsId
    }).getField('related_goods_id');
    let relatedGoods = null;
    if (think.isEmpty(relatedGoodsIds)) {
      // 查找同分类下的商品
      const goodsCategory = await model.where({
        id: goodsId
      }).find();
      relatedGoods = await model.where({
        category_id: goodsCategory.category_id
      }).field(['id', 'name', 'list_pic_url', 'retail_price']).limit(8).select();
    } else {
      relatedGoods = await model.where({
   ...
   */
  async wolfAction() {
    const user_name = this.get('name');
    const user_age = this.get('age');
    const user_tel = this.get('tel');
    const user_city = this.get('city');
    const user_company = this.get('company');
    const user_dec = this.get('dec');
    const user_mark = this.get('mark');
    // 添加到购物车
    const userWolfData = {
      user_name: user_name,
      user_age: user_age,
    ...
    // return this.success(userWolfData);

    const wolfAddInfo = await  this.model('wolf_user').add(userWolfData);
    return this.success(wolfAddInfo);
  }
